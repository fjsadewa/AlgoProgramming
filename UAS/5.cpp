...
int searching(struct data_pustaka arr[], int x, int r){
    for (a = 0; a < x; a++) {
        if (arr[a].kode == r) {
            return a; 
        }
...
int main () {
	...
	int pilihan_pertama,pilihan_kedua,ans,cr;
	
do{
	printf("\t\t\t Menu Daftar Pustaka\n");
	printf("1. Input Data \n2. Pengurutan Data \n3. Pencarian Data \n4. Terminated Program \n\n");
	...
	puts("");
		...
		system("cls");
	puts ("Ketikkan kode buku yang ingin di cari: ");
	scanf("%d",&cr);
	puts("");
    
	int ditemukan = searching(arr,data,cr);
    if(ditemukan==-1){
    printf("hasil\n");
    puts("");
    printf("error!!\n");
	...
	printf("Kode: %d\n",arr[ditemukan].kode);
	puts("");
	printf("Penulis :%s\n Judul:%s",arr[ditemukan].penulis,arr[ditemukan].judul);
	puts("");
	}
